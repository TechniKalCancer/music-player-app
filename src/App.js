...
import { Music, Play, Pause, SkipForward, SkipBack, Upload, Download, Edit3, Trash2, Clock, Users, LogOut, Plus, Save, X, Settings } from 'lucide-react';
...
const App = () => {
  ...
  // Track total play time and handle auto-pause
  useEffect(() => {
  ...
  // Handle silence countdown and auto-resume
  useEffect(() => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      schedules.forEach(schedule => {
        const shouldTrigger = schedule.time === currentTimeStr && 
  ...
  const handleSaveEdit = async () => {
    try {
      await axios.put(`/api/tracks/${editingTrack.id}`, {
  ...
  const moveTrack = async (index, direction) => {
    ...
    if (newIndex >= 0 && newIndex < playlist.length) {
      ...
      try {
        await axios.post('/api/tracks/reorder', { tracks: newPlaylist });
      } catch (error) {
  ...
  const saveSettings = async () => {
    try {
      await axios.put('/api/settings', {
        ...
        max_play_duration: maxPlayDuration
      });
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 bg-white/20 border border-white/30 rounded-lg text-white placeholder-purple-200 focus:outline-none focus:border-purple-400"
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-indigo-900 to-blue-900 p-4">
      ...
      {playlist.length > 0 && playlist[currentTrack] && (
        <audio
          ref={audioRef}
          src={`${window.location.origin}/uploads/${playlist[currentTrack].filename}`}
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            ...
            <div className="flex items-center gap-4 flex-wrap">
              <div className="text-purple-200 text-sm">
                <div className="font-semibold">{currentTime.toLocaleDateString()}</div>
          ...
          <div className="flex gap-2 flex-wrap">
            <button
              ...
              className={`px-4 py-2 rounded-lg font-semibold transition-colors ${
                activeView === 'player' ? 'bg-purple-600 text-white' : 'bg-white/20 text-purple-200 hover:bg-white/30'
              }`}
            >
              Player
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-lg font-semibold transition-colors ${
                activeView === 'playlist' ? 'bg-purple-600 text-white' : 'bg-white/20 text-purple-200 hover:bg-white/30'
              }`}
            >
              Manage Music
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-lg font-semibold transition-colors ${
                activeView === 'schedule' ? 'bg-purple-600 text-white' : 'bg-white/20 text-purple-200 hover:bg-white/30'
              }`}
            >
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg font-semibold transition-colors ${
                activeView === 'settings' ? 'bg-purple-600 text-white' : 'bg-white/20 text-purple-200 hover:bg-white/30'
              }`}
            >
            ...
            {currentUser.role === 'admin' && (
              <button
                ...
                className={`px-4 py-2 rounded-lg font-semibold transition-colors ${
                  activeView === 'admin' ? 'bg-purple-600 text-white' : 'bg-white/20 text-purple-200 hover:bg-white/30'
                }`}
              >
            ...
            <div className="space-y-6">
              ...
              <div className="flex justify-center items-center gap-4">
                <button
                  ...
                  disabled={isInSilencePeriod}
                  className="bg-purple-600 hover:bg-purple-700 text-white p-4 rounded-full transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                ...
                <button
                  ...
                  disabled={isInSilencePeriod}
                  className="bg-pink-600 hover:bg-pink-700 text-white p-6 rounded-full transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
              ...
              {isInSilencePeriod && (
                <div className="bg-yellow-500/20 border-2 border-yellow-500 rounded-lg p-4 text-center">
                  <div className="text-yellow-300 font-semibold text-lg mb-2">
                    🔇 Mandatory Silence Period
                  </div>
                  <div className="text-white text-2xl font-bold mb-1">
                    {formatSilence(silenceRemaining)}
                  </div>
              ...
              {/* Play Time Limit Display */}
              <div className="bg-white/10 p-4 rounded-lg">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-purple-200">Play Time Remaining</span>
                  <span className="text-white font-semibold">
                    {formatTime((maxPlayDuration * 60) - totalPlayTime)} / {formatDuration(maxPlayDuration)}
                  </span>
                </div>
                <div className="w-full bg-gray-700 rounded-full h-3 overflow-hidden">
                  <div 
                    className="bg-gradient-to-r from-purple-500 to-pink-500 h-full transition-all duration-1000"
                    style={{ width: `${(totalPlayTime / (maxPlayDuration * 60)) * 100}%` }}
                  ></div>
                </div>
                <div className="mt-2 text-purple-200 text-sm text-center">
                  After max duration, {formatSilence(silenceDuration)} of silence before auto-resume
                </div>
              </div>

              {/* Current Playlist */}
              <div className="mt-8">
                <h3 className="text-xl font-semibold text-white mb-4">Current Playlist</h3>
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {playlist.length === 0 ? (
                    <div className="text-center text-purple-200 py-8">
                      No tracks in playlist. Upload some music!
                    </div>
                  ) : (
          ...
          {activeView === 'playlist' && (
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                ...
                <label className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg cursor-pointer transition-colors">
              ...
              <div className="space-y-2">
                {playlist.length === 0 ? (
                  ...
                  playlist.map((track, idx) => (
                    <div key={track.id} className="bg-white/10 p-4 rounded-lg">
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          <div className="font-semibold text-white">{track.title}</div>
                          <div className="text-sm text-purple-200">{track.artist} • {track.duration}</div>
                        </div>
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => moveTrack(idx, 'up')}
                            disabled={idx === 0}
                            className="text-purple-300 hover:text-white disabled:opacity-30"
                          >
                            ▲
                          </button>
                          <button
              ...
              {editingTrack && (
                <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
                  <div className="bg-gray-800 rounded-xl p-6 w-full max-w-md">
                    <h3 className="text-xl font-bold text-white mb-4">Edit Track</h3>
                    <div className="space-y-4">
                      <div>
                      ...
                      <div>
                        <label className="block text-purple-200 mb-2">Artist</label>
                        <input
                          type="text"
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setNewSchedule({...newSchedule, time: e.target.value})}className="w-full px-4 py-2 bg-white/20 border border-white/30 rounded-lg text-white"
/>
</div>
              <div>
                <label className="block text-purple-200 mb-2">Days of Week</label>
                <div className="flex gap-2 flex-wrap">
              ...
              <div>
                <label className="block text-purple-200 mb-2">Action</label>
                <select
          ...
          <div className="space-y-2">
            ...
            {schedules.map(schedule => (
              <div key={schedule.id} className="bg-white/10 p-4 rounded-lg">
                <div className="flex justify-between items-start">
                  <div className="text-white">
                    ...
                    <div className="flex gap-1 flex-wrap">
                      {schedule.days.sort((a, b) => a - b).map(day => (
                        <span key={day} className="px-2 py-1 bg-white/20 rounded text-sm">
                          {getDayName(day)}
                        </span>
                      ))}
                    </div>
          ...
          <div className="bg-white/10 p-6 rounded-lg space-y-6">
            <div>
              <label className="block text-white font-semibold mb-2">
                Silence Between Tracks: {formatSilence(silenceDuration)}
              </label>
              <input
                type="range"
                min="0"
            ...
            <div>
              <label className="block text-white font-semibold mb-2">
                Max Play Duration: {formatDuration(maxPlayDuration)}
              </label>
              <input
                type="range"
                min="1"
                max="480"
                step="1"
                value={maxPlayDuration}
                onChange={(e) => setMaxPlayDuration(Number(e.target.value))}
                className="w-full"
              />
              <div className="flex justify-between text-sm text-purple-200 mt-1">
                <span>1m</span>
              ...
              <p className="text-sm text-purple-200 mt-2">
                Playback will automatically stop after this duration
              </p>
            </div>

            <div className="flex items-center justify-between">
              <div>
                <div className="text-white font-semibold">Fade In/Out Effects</div>
                <div className="text-sm text-purple-200">Smooth transitions between tracks</div>
              </div>
              <label className="relative inline-block w-14 h-8">
                ...
                />
                <div className="w-14 h-8 bg-gray-600 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-6 peer-checked:bg-purple-600 after:content-[''] after:absolute after:top-1 after:left-1 after:bg-white after:rounded-full after:h-6 after:w-6 after:transition-all"></div>
              </label>
            </div>
      ...
      {activeView === 'admin' && currentUser.role === 'admin' && (
        <div className="space-y-6">
          ...
          <div className="bg-white/10 p-4 rounded-lg">
            ...
            <div className="flex gap-2 flex-wrap">
              <input
                ...
                className="px-4 py-2 bg-white/20 border border-white/30 rounded-lg text-white placeholder-purple-200"
              />
              <input
                type="password"
          ...
          <div className="space-y-2">
            {users.map(user => (
              <div key={user.id} className="bg-white/10 p-4 rounded-lg flex justify-between items-center">
                <div className="text-white">
                  <div className="font-semibold">{user.username}</div>
                  <div className="text-sm text-purple-200">Role: {user.role}</div>
                </div>
                <button
